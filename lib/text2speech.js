...
class Text2Speech {
    ...
    yandexGetIAM(AuthToken) {
        ...
        if (now - lastAttempt > 36000000 || IAM === '') {
            ...
            const req = https.request(options, res => {
                this.adapter.log.debug('yandexGetIAM: Requesting new IAM token...');
                var body = '';
                ...
                res.on('end', () => {
                    try {
                        ...
                        this.adapter.log.debug('yandexGetIAM: IAM Received');
                    }
                    catch (e) {
                        this.adapter.log.error("yandexGetIAM: Response parse error");
                    }
                });
            }).on('error', err => this.adapter.log.error("yandexGetIAM: Error: " + err));

            req.on('error', error => this.adapter.log.error("yandexGetIAM: Error: " + error));

            req.write(postBody);
            req.end();
        } else {
            //this.adapter.log.info('yandexGetIAM: we still have actual IAM');
        };
    }
    ...
    async sayItGetSpeechYandexCloud(text, language, _, testOptions) {
        ...
        this.yandexGetIAM(yandexKey);
        
        //this.adapter.log.info('yandexKey=' + yandexKey);
        //this.adapter.log.info('IAM=' + IAM);
        //this.adapter.log.info('yandexFolderID=' + yandexFolderID);
