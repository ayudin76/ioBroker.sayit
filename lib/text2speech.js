    ...
    yandexGetIAM(AuthToken) {
        ...
        if (now - lastAttempt > 36000000 || IAM === '') {
            ...
            const req = https.request(options, res => {
                this.adapter.log.debug('Requesting new IAM token...');
                var body = '';
                ...
                res.on('end', () => {
                    try {
                        ...
                        this.adapter.log.debug('IAM Received');
                    }
                    catch (e) {
                        this.adapter.log.error("Response parse error");
                    }
                });
            }).on('error', err => this.adapter.log.error("Error: " + err));

            req.on('error', error => this.adapter.log.error("Error: " + error));

            req.write(postBody);
            req.end();
        } else {
            this.adapter.log.info('we still have actual IAM');
        };
    }
